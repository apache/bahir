...
object  WebHdfsConnector {
	...
	*/
	...
	def callWebHdfsAPI(path: String,  data: String, method: String, respType: String, trustStoreCredStr: String, usrCredStr: String, connStr: String, opr: String, outputType : String): Any = {
		...
		val uri = (if(trustCred._1 != "") "https:" else "http:") + pathComp(1) +  ":" + pathComp(2) + "?" + opr

		var httpc = Http(uri).auth(usrCred(0), usrCred(1)).timeout(connTimeoutMs = connProp(0), readTimeoutMs = connProp(1)) 
		...
		httpc = (trustCred._1 : @switch) match {
			...
			case "Y" => httpc.option(HttpOptions.sslSocketFactory(biocSslSocketFactory(trustCred._2, trustCred._3)))
		}
	...
	def getFileInputStream (filePath : String, offset : Long, length : Long, trustStoreCredStr: String, connStr: String, usrCredStr : String): ByteArrayInputStream = {
    
		//print("path in getFileInputStream : " + filePath + "\n")

		val fileGetOpr = if(length > 0) s"op=OPEN&offset=$offset&length=$length&bufferSize=$length" else  s"op=OPEN&offset=$offset"

		val getUrl = callWebHdfsAPI(filePath, "", "GET", "LOCATION", trustStoreCredStr, usrCredStr, connStr, fileGetOpr, "String").asInstanceOf[String]	

		val content = callWebHdfsAPI(getUrl, "", "GET", "BODY", trustStoreCredStr, usrCredStr, connStr, fileGetOpr, "Bytes").asInstanceOf[Array[Byte]]

		new ByteArrayInputStream(content)
  
	}	
	
	def getFileStatus(filePath: String, trustStoreCredStr : String, connStr: String, usrCredStr: String): Map[String, Any] = {

		//print("path in getFileStatus : " + filePath + "\n")
		val fileStatusOpr = s"op=GETFILESTATUS"	

		val fileStatus = callWebHdfsAPI(filePath, "", "GET", "BODY", trustStoreCredStr, usrCredStr, connStr, fileStatusOpr, "String").asInstanceOf[String]

		if (fileStatus.contains("RemoteException"))
        		throw new Exception(fileStatus)

		val responseMap = scala.util.parsing.json.JSON.parseFull(fileStatus).toList(0).asInstanceOf[Map[String, Map[String, Any]]]

		responseMap.getOrElse("FileStatus", throw new Exception ("File Status Null")) 

	}	

	def getListStatus(filePath: String, trustStoreCredStr : String, connStr: String, usrCredStr: String): List[Map[String, Any]] = {


		//print("path in getListStatus : " + filePath + "\n")
		val listStatusOpr = s"op=LISTSTATUS"	

		val listStatus = callWebHdfsAPI(filePath, "", "GET", "BODY", trustStoreCredStr, usrCredStr, connStr, listStatusOpr, "String").asInstanceOf[String]

		if (listStatus.contains("RemoteException"))
        		throw new Exception(listStatus)

		scala.util.parsing.json.JSON.parseFull(listStatus).toList(0).asInstanceOf[Map[String, Map[String, Any]]].get("FileStatuses").get("FileStatus").asInstanceOf[List[Map[String, Any]]]
	}	
