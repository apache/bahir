...
class DefaultSource
  ...
  override def createRelation(
	...
	val outRdd = (outputTypeStr : @switch) match {
		case "LIST" => WebHdfsConnector.listFromWebHdfs(sqlContext.sparkContext, path, trustCredStr, userCredStr, connPropStr)
		case default => WebHdfsConnector.loadFromWebHdfs(sqlContext.sparkContext, path, charset, trustCredStr, userCredStr, connPropStr, partitionDetailsStr, formatRecordSeparator)
	}

	val targetSchema = (outputTypeStr : @switch) match {
		case "LIST" =>  StructType(Array(StructField("Name",StringType,true),StructField("File Size",LongType,true),StructField("Block Size",LongType,true), StructField("# of Blocks", IntegerType, true)))
		case default => schema
	}
  ...
  override def createRelation(
	...
	/*
		* Writre now only CSV format is supported
	*/
	...
	WebHdfsConnector.writeToWebHdfs(rddToWrite, path, trustStoreCredStr, connStr, userCredStr, partitionStr)	
