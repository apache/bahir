...
private[pubsub] final case class ServiceAccountCredentials(
                                                            ...
                                                            jsonFileBuffer: Option[Array[Byte]] = None)
    ...
    if (!jsonFilePath.isEmpty) {
      ...
      CredentialFactory.GoogleCredentialWithRetry.fromGoogleCredential(
        GoogleCredential.fromStream(stream, transport, jsonFactory)
          .createScoped(scopes))}
    else if (jsonFileBuffer match {
      case Some(arrayByte) => true
      case None => false
    }) {
      val stream = new ByteArrayInputStream(fileBytes)

      val creds = GoogleCredential.fromStream(
        new ByteArrayInputStream(fileBytes),
        transport, jsonFactory)
      CredentialFactory.GoogleCredentialWithRetry.fromGoogleCredential(creds.createScoped(scopes))}
  ...
  /*  private def getStream (stream: InputStream): InputStream = stream*/

  private def getFileBuffer: Array[Byte] = {
    val filePath = jsonFilePath orElse p12FilePath
    if (filePath.isEmpty) {
      jsonFileBuffer match {
        case Some(arrayByte) => arrayByte
...
object SparkGCPCredentials {
  ...
  class Builder {
    ...
    /**
      * Use a Array[Byte] of json file for service account credential
      *
      * @param jsonFileBuffer Array[Byte] of json file
      * @return Reference to this SparkGCPCredentials.Builder
      */
    def jsonServiceAccount(jsonFileBuffer: Array[Byte]): Builder = {
      creds = Option(ServiceAccountCredentials(None, None, None, Option(jsonFileBuffer)))
      this
    }
    /**
